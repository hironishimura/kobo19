...
function kobo19_app_source() {
	return array(
		...
		'meta'    => array(
			...
			'store'    => '',
			// Mac 版の配布ファイル。メディアに上げた ZIP の URL を入れると、製品ページに
			// ダウンロードのボタンと入れかたが出る（本番は管理画面から設定する）
			'download' => '',
			'download_note' => 'iPhone / iPad と同じ Apple アカウントで iCloud にサインインしていれば、書類は自動で同期されます。',
			'version'  => '1.0',
...
`plot` と書けば曲線が、`chart` と書けば棒グラフが、その式のすぐ下に出ます。式に `x` と `y` の両方があれば曲面になり、横になぞると回せます。打ち直すたびにその場で描き直されます。
